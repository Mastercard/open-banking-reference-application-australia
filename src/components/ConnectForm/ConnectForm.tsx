...
export default function ConnectForm() {
    ...
    const handleNext = (skip = false) => {
        ...
        if (!skip) {
            ...
            handleError({
                message:
                    'Failed to retrieve consent receipt Id: Please try again after creating new credentials (Partner Id, Partner Secret and App Key)',
            });
        }
    ...
    const getAccountInformation = async (customerId: string) => {
        try {
            const consent = await retrieveConsent(subscriptionUuid, customerId);
            if (!consent) {
                return 'skipnext';
            }
            const requestHeaders = generateFetchHeaders('GET', appToken);
            requestHeaders.headers.append('Consent-Receipt-Id', consent);
    ...
    const getAccountACHInformation = (
        accounts: any,
        customerId: any,
        consent: string
    ) => {
        ...
        return depositAccounts.map(({ id }: any) => {
            return fetch(
                ...
                .catch((error) => {
                    console.error('error', error);
                    if (
                        RegExp(/Account type not supported/i).test(
                            error.message
                        )
                    ) {
                        return [];
